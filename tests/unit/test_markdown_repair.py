...
def test_repairs_orphan_outer_group_before_matrix() -> None:
    source = (
        r"{\begin{matrix} P_e = 1.5(v_d i_{gd}+v_q i_{gq}) "
        r"\\ Q_e = 1.5(v_q i_{gd}-v_d i_{gq}) \end{matrix}"
    )
...
def test_does_not_guess_at_internal_brace_damage() -> None:
    source = r"\frac{a}{b"
    repaired, changed, validation = repair_latex_structure(source)
...
def test_localizes_and_deduplicates_ieee_large_small_figure(
    tmp_path: Path,
) -> None:
...
def test_repairs_display_formula_inside_markdown(tmp_path: Path) -> None:
    markdown_path = tmp_path / "paper.md"
...
def test_code_fence_is_not_modified(tmp_path: Path) -> None:
    markdown_path = tmp_path / "paper.md"
    markdown = """```latex
$$
